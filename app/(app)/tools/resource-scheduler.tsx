...
} from '@/lib/resourceScheduler/repository';
import {
...
export default function ResourceSchedulerScreen() {
  const { t } = useI18n();
  ...
  const itemsQuery = useQuery({
    queryKey: ['resource-scheduler', uid, selectedProjectId],
    queryFn: () => listResourceBookings(selectedProjectId!),
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((b) => b.kind === kindFilter);
  }, [items, kindFilter]);
  ...
  const saveMut = useMutation({
    mutationFn: async () => {
      ...
      if (!name.trim() || !startDate.trim() || !endDate.trim()) {
        throw new Error('Enter resource name, start date, and end date.');
      }
      ...
      if (editingId) await updateResourceBooking(selectedProjectId, editingId, row);
      else await addResourceBooking(selectedProjectId, row);
    },
    onSuccess: () => {
      invalidate();
      closeForm();
  ...
  const deleteMut = useMutation({
    mutationFn: ({ pid, id }: { pid: string; id: string }) => deleteResourceBooking(pid, id),
  ...
  const openEdit = (b: ResourceBooking) => {
    setEditingId(b.id);
    setKind(b.kind);
    setName(b.name);
    setQuantityLabel(b.quantityLabel);
                ...
                <View className="pb-3">
                  ...
                  <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                    <View className="flex-row gap-2">
                      {(['all', ...RESOURCE_KINDS] as const).map((f) => (
                        <Pressable
                          key={f}
                          onPress={() => setKindFilter(f)}
                          className="rounded-full border px-3 py-1.5"
                          style={{
                            borderColor: kindFilter === f ? Colors.brand[700] : Colors.neutral[300],
                            backgroundColor: kindFilter === f ? Colors.brand[100] : '#fff',
                          }}
                        >
              ...
              >
                <View className="flex-row items-start justify-between">
                  <View className="mr-2 flex-1">
                    <Text
                      className="self-start rounded-full bg-neutral-100 px-2 py-0.5 text-[11px] text-neutral-800"
              ...
              <View className="mb-3 flex-row flex-wrap gap-2">
                {RESOURCE_KINDS.map((k) => (
                  <Pressable
                    ...
                    className="rounded-full border px-3 py-1.5"
                    style={{
                      borderColor: kind === k ? Colors.brand[700] : Colors.neutral[300],
                      backgroundColor: kind === k ? Colors.brand[100] : '#fff',
                    }}
                  >
                    <Text className="text-xs text-brand-900" style={{ fontFamily: 'Inter_500Medium' }}>
