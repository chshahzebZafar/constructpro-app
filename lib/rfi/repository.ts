...
export async function listRfis(projectId: string): Promise<RfiItem[]> {
  ...
  const rows = await loadProjectArrayOrMigrate<RfiItem>(u, projectId, TOOL_KEYS.rfi, loadBlob, saveBlob);
  return sortRfis(rows);
}
...
export async function addRfi(projectId: string, row: Omit<RfiItem, 'id'>): Promise<RfiItem> {
  ...
  const rows = await loadProjectArrayOrMigrate<RfiItem>(u, projectId, TOOL_KEYS.rfi, loadBlob, saveBlob);
  rows.push(item);
  await saveProjectArraySnapshot(u, projectId, TOOL_KEYS.rfi, rows);
  return item;
}
